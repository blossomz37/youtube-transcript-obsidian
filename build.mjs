...
await build({ entryPoints: [new URL('./src/main.js', import.meta.url).pathname],
  outfile: new URL('main.js', out).pathname, bundle: true, platform: 'node',
  format: 'cjs', target: 'es2022', external: ['obsidian'], loader: { '.md': 'text' },
  logLevel: 'info' });
for (const name of ['manifest.json', 'styles.css']) await copyFile(new URL(name, import.meta.url), new URL(name, out));
